import React from "react";
import * as _Builtin from "./_Builtin";
import * as _utils from "./utils";
import _styles from "./HammerNailsSection.module.css";

export function HammerNailsSection({ as: _Component = _Builtin.Section }) {
  return (
    <_Component
      className={_utils.cx(_styles, "section-2")}
      grid={{
        type: "section",
      }}
      tag="section"
    >
      <_Builtin.Block
        className={_utils.cx(_styles, "container-1600")}
        tag="div"
      >
        <_Builtin.Heading className={_utils.cx(_styles, "heading-1")} tag="h1">
          {"HAMMER&NAILS PAC"}
        </_Builtin.Heading>
        <_Builtin.Block className={_utils.cx(_styles, "text-block")} tag="div">
          {"Become a Key Member Today!"}
          <br />
        </_Builtin.Block>
        <_Builtin.Block className={_utils.cx(_styles, "home-grid")} tag="div">
          <_Builtin.Block
            className={_utils.cx(_styles, "home-grid-left")}
            id={_utils.cx(
              _styles,
              "w-node-d07a64ae-1877-314a-0f4c-4c9c8fc48de5-8fc48ddd"
            )}
            tag="div"
          >
            <_Builtin.Block
              className={_utils.cx(_styles, "main-left-list")}
              tag="div"
            >
              <_Builtin.Image
                className={_utils.cx(_styles, "list-logo-left")}
                loading="lazy"
                width="auto"
                height="auto"
                alt=""
                src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a853b0fad09343073a4c7_gfwba-logo-vert.png"
              />
              <_Builtin.Block
                className={_utils.cx(_styles, "p-main")}
                tag="div"
              >
                {
                  "The Hammer and Nails PAC is an important element of the GFWBA’s government affairs program, and is the political voice for the greater Fort Worth home building industry. The committee contributes money to local and state candidates who understand and support the efforts of the building industry, and organize effective political action on behalf of our members and the home building industry."
                }
              </_Builtin.Block>
            </_Builtin.Block>
            <_Builtin.Block
              className={_utils.cx(_styles, "p-main", "space-20")}
              tag="div"
            >
              {
                "The Hammer and Nails PAC offers opportunities for GFWBA members to interact with and build relationships with local, state and federal officials. You can have a voice in electing candidates who support the building industry. Interesting in getting involved? Become a part of the Hammer and Nails PAC!"
              }
            </_Builtin.Block>
            <_Builtin.List
              className={_utils.cx(_styles, "space-40")}
              tag="ul"
              unstyled={false}
            >
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {
                    "Corporate Contributions – Corporate contributions are used to pay for events that raise funds for local and state candidates."
                  }
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {
                    "Personal Contributions – Contributions to local and state candidates who support the efforts of the building industry."
                  }
                  <br />
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Capitol Club —  $5000 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Diamond Key — $3000 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Platinum Key — $1500 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Gold Key — $1000 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Silver Key — $500 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Bronze Key — $250 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Lone Star — $100 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  {"Hammer and Nails PAC Member — $30 annually"}
                </_Builtin.Block>
              </_Builtin.ListItem>
            </_Builtin.List>
          </_Builtin.Block>
          <_Builtin.Block
            className={_utils.cx(_styles, "home-grid-right")}
            id={_utils.cx(
              _styles,
              "w-node-d07a64ae-1877-314a-0f4c-4c9c8fc48e0c-8fc48ddd"
            )}
            tag="div"
          >
            <_Builtin.Heading
              className={_utils.cx(_styles, "heading-2")}
              tag="h2"
            >
              {"ASSOCIATION STAFF"}
            </_Builtin.Heading>
            <_Builtin.Block
              className={_utils.cx(_styles, "staff-item-blk")}
              tag="div"
            >
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Kimberly Eaton-Pregler"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Executive Vice President"}
                </_Builtin.Block>
              </_Builtin.Block>
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Sharon Liles Love"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Director of Operations"}
                </_Builtin.Block>
              </_Builtin.Block>
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Shannon Claybaugh"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Director of Member Services/"}
                  <br />
                  {"Certified Master Builder Administrator"}
                </_Builtin.Block>
              </_Builtin.Block>
            </_Builtin.Block>
          </_Builtin.Block>
        </_Builtin.Block>
      </_Builtin.Block>
    </_Component>
  );
}
