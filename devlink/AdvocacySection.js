"use client";
import React from "react";
import * as _Builtin from "./_Builtin";
import * as _utils from "./utils";
import _styles from "./AdvocacySection.module.css";

export function AdvocacySection({ as: _Component = _Builtin.Section }) {
  return (
    <_Component
      className={_utils.cx(_styles, "section-2")}
      grid={{
        type: "section",
      }}
      tag="section"
    >
      <_Builtin.Block
        className={_utils.cx(_styles, "container-1600")}
        tag="div"
      >
        <_Builtin.Heading className={_utils.cx(_styles, "heading-1")} tag="h1">
          {"GOVERNMENTAFFAIRS"}
        </_Builtin.Heading>
        <_Builtin.Block className={_utils.cx(_styles, "text-block")} tag="div">
          {"Advocacy - Click a link below to learn more"}
          <br />
        </_Builtin.Block>
        <_Builtin.Block className={_utils.cx(_styles, "home-grid")} tag="div">
          <_Builtin.Block
            className={_utils.cx(_styles, "home-grid-left")}
            id={_utils.cx(
              _styles,
              "w-node-_0eceecc8-4bd0-bf75-f114-313e1fba39a2-1fba399a"
            )}
            tag="div"
          >
            <_Builtin.Block
              className={_utils.cx(_styles, "main-left-list")}
              tag="div"
            >
              <_Builtin.Image
                className={_utils.cx(_styles, "list-logo-left")}
                loading="lazy"
                width="auto"
                height="auto"
                alt=""
                src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a853b0fad09343073a4c7_gfwba-logo-vert.png"
              />
              <_Builtin.Block
                className={_utils.cx(_styles, "p-main", "border-green")}
                tag="div"
              >
                {
                  "Lorem ipsum dolor sit amet, consectetur adipiscing elit. Donec eget blandit dolor, vitae faucibus est. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nulla et elit sodales, porttitor lectus non, congue mauris. Vivamus et risus nibh. Vestibulum tempor vestibulum varius. Vestibulum massa diam, consectetur sed lobortis nec, ornare et est. In condimentum, enim at convallis lacinia, lectus urna blandit nunc, nec tempus leo lorem posuere sapien."
                }
              </_Builtin.Block>
            </_Builtin.Block>
            <_Builtin.List
              className={_utils.cx(_styles, "list-space-40", "list-dec")}
              tag="ul"
              unstyled={false}
            >
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    target="_blank"
                    block=""
                    options={{
                      href: "https://gfwbatx.com/resources/Documents/2022/State%20Advocacy%20in%20Action.pdf",
                    }}
                  >
                    {"State Advocacy in Action"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    target="_blank"
                    block=""
                    options={{
                      href: "https://gfwbatx.com/resources/Documents/2022/FINAL%20BRS%20GFWBA%20Summary.pdf",
                    }}
                  >
                    {"2022 Summary of Supreme Court Case"}
                  </_Builtin.Link>
                  {""}
                  <br />
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#HB852",
                    }}
                  >
                    {"HB 852 You Pay Less for Permits"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#HB2439",
                    }}
                  >
                    {
                      "HB 2439 – You Can Use All Building Materials Approved by Code Without Arbitrary Limitations from Municipalities"
                    }
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#HB1743",
                    }}
                  >
                    {"HB 1743 – You Pay Less in Rollback Taxes"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#HB3167",
                    }}
                  >
                    {"HB 3167 Faster Plan Approval"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments#TAXESON",
                    }}
                  >
                    {"Tax Reform Victories for Housing"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments#CITYOF",
                    }}
                  >
                    {"City of Fort Worth iSWMRegulations"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#WOTUS",
                    }}
                  >
                    {
                      "WOTUS/Navigable Waters Protection Rule - Fewer Federal Permitting Requirements"
                    }
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments#IMPACT",
                    }}
                  >
                    {"Impact Fees"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments#LOCAL",
                    }}
                  >
                    {"Local Issues"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
              <_Builtin.ListItem>
                <_Builtin.Block
                  className={_utils.cx(_styles, "p-main")}
                  tag="div"
                >
                  <_Builtin.Link
                    className={_utils.cx(_styles, "p-main-link")}
                    button={false}
                    block=""
                    options={{
                      href: "https://gfwbatx.com/Accomplishments/#WEHELP",
                    }}
                  >
                    {"We Help Keep the Doors Open"}
                  </_Builtin.Link>
                  {""}
                </_Builtin.Block>
              </_Builtin.ListItem>
            </_Builtin.List>
          </_Builtin.Block>
          <_Builtin.Block
            className={_utils.cx(_styles, "home-grid-right")}
            id={_utils.cx(
              _styles,
              "w-node-_0eceecc8-4bd0-bf75-f114-313e1fba39e5-1fba399a"
            )}
            tag="div"
          >
            <_Builtin.Heading
              className={_utils.cx(_styles, "heading-2")}
              tag="h2"
            >
              {"ASSOCIATIONSTAFF"}
            </_Builtin.Heading>
            <_Builtin.Block
              className={_utils.cx(_styles, "staff-item-blk")}
              tag="div"
            >
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Kimberly Eaton-Pregler"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Executive Vice President"}
                </_Builtin.Block>
              </_Builtin.Block>
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Sharon Liles Love"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Director of Operations"}
                </_Builtin.Block>
              </_Builtin.Block>
              <_Builtin.Block
                className={_utils.cx(_styles, "staff-item")}
                tag="div"
              >
                <_Builtin.Image
                  className={_utils.cx(_styles, "", "image-4")}
                  loading="lazy"
                  width="auto"
                  height="auto"
                  alt=""
                  src="https://uploads-ssl.webflow.com/6549729854ffbc32e05dfa25/655a86cc1de34c4d81716a2e_filller.png"
                />
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-name")}
                  tag="div"
                >
                  {"Shannon Claybaugh"}
                </_Builtin.Block>
                <_Builtin.Block
                  className={_utils.cx(_styles, "staff-title")}
                  tag="div"
                >
                  {"Director of Member Services/"}
                  <br />
                  {"Certified Master Builder Administrator"}
                </_Builtin.Block>
              </_Builtin.Block>
            </_Builtin.Block>
          </_Builtin.Block>
        </_Builtin.Block>
      </_Builtin.Block>
    </_Component>
  );
}
